...
export class ProductoCatalogService {
  ...
  /**
   * Genera códigos únicos para un producto
   * Usa transacción para evitar race conditions
   * @param empresaId ID de la empresa
   * @param sedeId ID de la sede (opcional)
   * @param tx Transacción de Prisma (opcional) - si se proporciona, se ejecuta dentro de esa transacción
   */
  async generateCodigos(
    empresaId: string,
    sedeId?: string,
    tx?: Prisma.TransactionClient,
  ): Promise<{ codigoEmpresa: string; codigoSistema: string }> {
    // Si ya hay una transacción, usarla directamente sin crear otra
    if (tx) {
      return await this._generateCodigosInTransaction(tx, empresaId, sedeId);
    }

    // Si no hay transacción, crear una nueva
    return await this.prisma.$transaction(async (txInner) => {
      return await this._generateCodigosInTransaction(txInner, empresaId, sedeId);
    });
  }

  /**
   * Lógica interna de generación de códigos (usada dentro de transacción)
   */
  private async _generateCodigosInTransaction(
    tx: Prisma.TransactionClient,
    empresaId: string,
    sedeId?: string,
  ): Promise<{ codigoEmpresa: string; codigoSistema: string }> {
      ...
      if (existe) {
        ...
        return this._generateCodigosInTransaction(tx, empresaId, sedeId);
      }
  ...
  async generateCodigoVariante(
    ...
    // Si ya hay una transacción, usarla directamente sin crear otra
    if (tx) {
      return await this._generateCodigoVarianteInTransaction(tx, empresaId);
    }

    // Si no hay transacción, crear una nueva
    return await this.prisma.$transaction(async (txInner) => {
      return await this._generateCodigoVarianteInTransaction(txInner, empresaId);
    });
  }

  /**
   * Lógica interna de generación de código de variante (usada dentro de transacción)
   */
  private async _generateCodigoVarianteInTransaction(
    tx: Prisma.TransactionClient,
    empresaId: string,
  ): Promise<{ codigoEmpresa: string }> {
    // Obtener o crear configuración de códigos
    ...
    if (existe) {
      ...
      return this.generateCodigoVariante(empresaId, true, tx);
    }
